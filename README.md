...
> Status: **Firmware-owned MEM_COPY, MEM_FILL, and CRC32 implemented.** The
...
> Firmware-validated `MEM_COPY`, `MEM_FILL`, and `CRC32` now perform checked PCI
> payload DMA with exact byte counts, guarded writes, and verified CRC results.
...
- Firmware-owned `MEM_COPY`, `MEM_FILL`, and `CRC32` validation with QEMU PCI DMA
  execution
...
- `MEM_COPY`, `MEM_FILL`, and `CRC32` use a synchronous correctness-first QEMU
  engine. Vector add, asynchronous execution, engine registers, and host
  telemetry are not implemented.
- The endpoint retains a direct validator only for isolated QTests; integrated
  NOP, MEM_COPY, MEM_FILL, and CRC32 commands use real Zephyr validation.
