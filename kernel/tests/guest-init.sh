...
finish()
{
	code="$1"
	if [ "$code" -eq 0 ]; then
		echo 'VAMS Linux PCI UAPI, queue, polling, and cleanup smoke test: PASS'
	else
		echo 'VAMS Linux PCI UAPI, queue, polling, or cleanup smoke test: FAIL'
	fi
	sync
	devmem 0xf4 32 "$code"
	poweroff -f
...
insmod /vams_pci.ko probe_irq_selftest=1 probe_nop_selftest=1 \
	...
	fail 'normal probe, MSI-X, NOP, or polling self-test failed'
...
dmesg | grep -q 'CQ polling fallback self-test passed' ||
	fail 'CQ polling fallback was not observed'
[ -c /dev/vams0 ] || fail 'VAMS host API device node is absent'
/vams-uapi-test /dev/vams0 || fail 'VAMS host API round trip failed'
