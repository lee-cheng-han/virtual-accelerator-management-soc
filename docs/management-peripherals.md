...
The `vams-mgmt` QEMU device provides two firmware-facing 4 KiB MMIO regions:
a mailbox at `0x10010000` and a watchdog/reset/telemetry block at
`0x10020000`. All defined registers are naturally aligned, little-endian
32-bit accesses. Other widths, unaligned accesses, and undefined offsets are
invalid.
...
Three focused tests cover the contract:
...
requires continued healthy telemetry with no assertion or trap report.
...
- Migration state is implemented for the peripheral, but live migration is not
  an accepted or tested platform capability.
