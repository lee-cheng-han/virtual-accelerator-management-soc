...
test, exposed as `mem-copy-smoke`, `mem-fill-smoke`, `crc32-smoke`, and
`vector-add-smoke`, verifies odd-length patterned copy and fill operations, an
independently computed IEEE CRC32, matching and mismatching expected results,
little-endian vector arithmetic with unsigned wraparound, exact byte counts,
